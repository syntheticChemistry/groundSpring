...
//! Typed accessors for benchmark JSON fields.
//!
//! Two API layers:
//! - **Fallible** (`get_*`): Return `Result<T, BenchFieldError>` for callers
//!   that handle missing fields gracefully.
//! - **Panicking** (`*_field`): Wrap `get_*` with `.expect()` for validation
//!   binaries where JSON is `include_str!`-ed at compile time and a missing
//!   field is a programmer error, not a runtime condition.
...
pub fn get_usize(v: &Value, key: &str) -> BenchResult<usize> {
    v.get(key)
        ...
        .map(groundspring::cast::u64_usize)
        .ok_or_else(|| BenchFieldError {
            field: key.into(),
            expected: "u64 (usize)",
        })
...
//
// Delegate to the `get_*` functions and panic on `Err`. Suitable
