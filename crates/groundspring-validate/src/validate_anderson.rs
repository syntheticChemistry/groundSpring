...
fn main() {
    ...
    let nonzero_gammas: Vec<f64> = nonzero.iter().map(|(_, g)| *g).collect();
    ...
    let gamma_8 = gammas.iter().find(|(w, _)| (*w - 8.0).abs() < 0.01).unwrap().1;
    h.check_min(
    ...
    let gamma_1 = gammas.iter().find(|(w, _)| (*w - 1.0).abs() < 0.01).unwrap().1;
    let xi_1 = localization_length(gamma_1);
    let c = xi_1;

    println!("  At W=1: ξ={xi_1:.1}, C = ξ·W² = {c:.1}");
    ...
    // ── Part 4: Localization length decreases ─────────────────────────
    println!("\n--- Part 4: Localization Length vs Disorder ---");

    let xi_values: Vec<f64> = nonzero_gammas.iter().map(|g| localization_length(*g)).collect();
