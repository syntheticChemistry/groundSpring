...
/// `λ_max` = (1 + √γ)². This is the cutoff used by [`spectral_diagnostics`]
/// for outlier-based phase classification.
///
/// When the `barracuda` feature is enabled, delegates to
...
pub fn detect_transition(sweep: &[SweepPoint]) -> Option<f64> {
    ...
    // Compute |d⟨r⟩/dW| via finite differences — peak marks transition.
    ...
    // CPU fallback: simple argmax of |derivative|.
    let peak_idx = deriv
        .iter()
        .enumerate()
...
mod tests {
    ...
    fn esd_uniform_eigenvalues() {
        ...
        // All bins should have roughly equal density for uniform data.
        for d in &densities {
            assert!(
                *d > 0.0,
    ...
    fn detect_transition_monotone_sweep() {
        // Simulated sweep: ⟨r⟩ drops sharply near W=3.0
