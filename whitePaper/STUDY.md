...
- **Phase 2a (DONE)**: Tier A rewire — **87 active delegations (51 CPU + 36 GPU), 1 evolution candidate — toadStool S94b**. barracuda from `barraCuda` primal (`ecoPrimals/barraCuda/`). GPU stats dispatch (mean, std_dev, rmse, mbe, pearson_r) + batch GPU APIs (GillespieGpu, WrightFisherGpu, BatchedElementwiseF64). Rust is **11.5× faster** than Python (excl. LAPACK-bound; Exp 009: 47.7× from Sturm tridiag). 29/29 parity proven. V51: 752 workspace tests, 376/376 checks, 30 metalForge workloads (24 GPU + 2 NPU + 2 CPU-only), 100+ three-tier parity tests, 9 CPU vs GPU parity tests
