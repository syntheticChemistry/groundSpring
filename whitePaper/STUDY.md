...
- **Phase 2a (DONE)**: Tier A rewire — **73 active delegations (43 CPU + 30 GPU), 1 evolution candidate — ToadStool S86**. GPU stats dispatch (mean, std_dev, rmse, mbe, pearson_r) + batch GPU APIs (GillespieGpu, WrightFisherGpu, BatchedElementwiseF64). Rust is **11.5× faster** than Python (excl. LAPACK-bound; Exp 009: 47.7× from Sturm tridiag). 28/28 parity proven. V51: 752 workspace tests, 376/376 checks, 28 metalForge workloads (22 GPU + 2 NPU + 2 CPU-only), 95 three-tier parity tests, 9 CPU vs GPU parity tests
