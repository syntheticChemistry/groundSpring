...
**NUCLEUS**: biomeOS Neural API live (V108) — Tower, Node, Squirrel validated; NestGate data pipelines (NCBI, NOAA, IRIS) with sovereign fallback. Adaptive health probing, direct primal discovery. Typed capability-based discovery (`discover_by_capability`, `compute_execute`, `storage_put/get`). Typed `BiomeOsError` enum (7 variants). `primal_names.rs` centralized constants.
