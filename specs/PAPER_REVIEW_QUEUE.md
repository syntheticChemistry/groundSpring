...
**Barracuda**: 73 active delegations (43 CPU + 30 GPU) — ToadStool S86. **Performance**: 11.6× faster than Python (excl. LAPACK-bound); 5.1× overall; 53.5× peak (seismic). **Tests**: 776 Rust workspace + 375 Python = 1151. 100+ three-tier parity tests (100% delegation coverage).
...
**GPU tier**: 13 modules wired with `#[cfg(feature = "barracuda-gpu")]` — stats Tier A complete (MAE, NSE, R²), bistable batch ODE. 28 metalForge workloads (22 GPU). GPU grid adapters (seismic, freeze-out). 316/322 tests pass (6 require f64-capable GPU: Titan V / A100).
...
73 active delegations (43 CPU + 30 GPU) — ToadStool S86 (`7e01ac7e`). 11.5× faster than Python (excl. LAPACK-bound).
776 Rust workspace tests + 375 Python = 1151. 28/28 mathematical parity proven. 100+ three-tier parity tests (100% delegation coverage).
376/376 validation checks across 33 experiments (V66, zero-debt audit certified).
All 28 experiments validated. GPU stats dispatch (mean, std_dev, rmse, mbe, mae, nse, r², pearson_r). 14 CPU vs GPU parity tests. CPU vs GPU benchmark binary.
